...
// Update order status
export const updateOrderStatus = async (req: Request<{ id: string }, {}, UpdateOrderStatusRequest>, res: Response): Promise<void> => {
  try {
    const { id } = req.params;
...
export const updateBook = async (req: Request<{ id: string }, {}, UpdateBookRequest>, res: Response): Promise<void> => {
  try {
    const { id } = req.params;
    const { title, author, publisher, price, numericPrice, image, pdfUrl } = req.body;
    ...
    if (numericPrice) updateData.numericPrice = parseFloat(numericPrice.toString());
